...
/obj/structure/roguemachine/mail/proc/build_sanitized_letter(mob/user, sender, recipient, content)
	...
	if(!(findtext(recipient,"#")==1))
		recipient = reject_bad_name(recipient, max_length=MAX_CHARTER_LEN) // reject_bad_name doesn't let you use #s in names, so we only apply this if it's NOT a hermes number
	...
	if(istype(P, /obj/item/paper) || istype(P, /obj/item/smallDelivery))
		...
		if(alert(user, "Send Mail?",,"YES","NO") == "YES")
			...
			if(!(findtext(send2place,"#")==1))
				send2place = reject_bad_name(send2place, max_length=MAX_CHARTER_LEN) // reject_bad_name doesn't let you use #s in names, so we only apply this if it's NOT a hermes number
