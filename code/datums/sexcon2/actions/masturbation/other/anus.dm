...
/datum/sex_action/masturbate/other/anus/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.visible_message(span_warning("[user] starts [do_subtle ? "subtly " : ""]fingering [target]'s butt..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))

/datum/sex_action/masturbate/other/anus/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	. = ..()
	var/datum/sex_session/sex_session = get_sex_session(user, target)
	var/do_subtle = sex_session.doing_subtly
	user.visible_message(span_warning("[user] stops [do_subtle ? "subtly " : ""]fingering [target]'s butt."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
