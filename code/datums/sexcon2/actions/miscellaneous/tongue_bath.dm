...
/datum/sex_action/miscellaneous/tonguebath/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.visible_message(span_warning("[user] [do_subtle ? "subtly " : ""]sticks [user.p_their()] tongue out, getting close to [target]..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))

/datum/sex_action/miscellaneous/tonguebath/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	. = ..()
	var/datum/sex_session/sex_session = get_sex_session(user, target)
	var/do_subtle = sex_session.doing_subtly
	user.visible_message(span_warning("[user] stops [do_subtle ? "subtly " : ""]bathing [target]'s body ..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/miscellaneous/tonguebath/on_perform_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(check_location_accessible(user, target, BODY_ZONE_CHEST, TRUE))
		...
		body_desc = "exposed body"
	user.visible_message(sex_session.spanify_force("[user] [sex_session.get_generic_force_adjective(do_subtle)] bathes [target]'s [body_desc] with [user.p_their()] tongue..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
