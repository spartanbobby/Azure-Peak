...
/datum/sex_action/oral/armpit_nuzzle/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.visible_message(span_warning("[user] [do_subtle ? "subtly " : ""]moves [user.p_their()] head against [target]'s armpit..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))

/datum/sex_action/oral/armpit_nuzzle/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	. = ..()
	var/datum/sex_session/sex_session = get_sex_session(user, target)
	var/do_subtle = sex_session.doing_subtly
	user.visible_message(span_warning("[user] stops [do_subtle ? "subtly " : ""]nuzzling [target]'s armpit..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/oral/armpit_nuzzle/on_perform_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	var/datum/sex_session/sex_session = get_sex_session(user, target)
	var/do_subtle = sex_session.doing_subtly
	user.visible_message(sex_session.spanify_force("[user] [sex_session.get_generic_force_adjective(do_subtle)] nuzzles [target]'s armpit..."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
