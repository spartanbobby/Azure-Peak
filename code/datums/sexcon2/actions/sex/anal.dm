...
/datum/sex_action/sex/anal/on_perform_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.visible_message(sex_session.spanify_force("[user] [sex_session.get_generic_force_adjective(do_subtle)] [is_knotting ? "knot-fucks" : "fucks"] [target]'s ass."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/sex/anal/double/on_perform_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	var/do_subtle = sex_session.doing_subtly
	var/is_knotting = sex_session.do_knot_action
	user.visible_message(sex_session.spanify_force("[user] [sex_session.get_generic_force_adjective(do_subtle)] [is_knotting ? "double-knots" : "double-fucks" ] [target]'s ass."), vision_distance = (do_subtle ? 2 : DEFAULT_MESSAGE_RANGE))
